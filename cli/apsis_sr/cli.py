...
def sign_request(method: str, url: str, payload: bytes, region: str, profile: Optional[str]) -> dict[str, str]:
    """Sign request with AWS Signature Version 4."""
    # Get credentials
    creds_data = aws_json(["sts", "get-session-token"], profile=profile, region=region)
...
def call_devbox_api(api_url: str, action: str, user_id: str, region: str, profile: Optional[str]) -> dict:
    url = api_url.rstrip("/") + "/devbox"
    payload = json.dumps({"action": action, "userId": user_id}).encode("utf-8")
    headers = sign_request("POST", url, payload, region, profile)
...
def get_devbox_status(profile: Optional[str], region: str, stack_name: str, user_id: str) -> dict:
    ...
    return call_devbox_api(api_url, "status", user_id, region, profile)
...
def cmd_devbox_provision(args: argparse.Namespace) -> None:
    ...
    response = call_devbox_api(api_url, "provision", user_id, args.region, args.profile)
...
def cmd_devbox_stop(args: argparse.Namespace) -> None:
    user_id = args.user_id or get_user_id(args.profile)
    api_url = get_api_url(args.profile, args.region, args.stack_name)
    response = call_devbox_api(api_url, "stop", user_id, args.region, args.profile)
    print(json.dumps(response, indent=2))
...
def cmd_devbox_terminate(args: argparse.Namespace) -> None:
    ...
    user_id = args.user_id or get_user_id(args.profile)
    api_url = get_api_url(args.profile, args.region, args.stack_name)
    response = call_devbox_api(api_url, "terminate", user_id, args.region, args.profile)
    print(json.dumps(response, indent=2))
